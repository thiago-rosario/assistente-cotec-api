...
namespace App\Core\Conversation\Infra\External;
...
class PythonWhatsappMessageBridge
{
    ...
    {
        return PythonWhatsappMessageBridge . phpjson_encode([
                'type' => 'send_message',
                'payload' => [
    ...
    public function command(string $pythonBinary = 'python3', ?string $scriptPath = null): array
    {
        return [
            $pythonBinary,
            $scriptPath ?? base_path('src/Conversation/Infra/External/Python/main.py'),
            '--bridge-output=json',
        ];
    }
